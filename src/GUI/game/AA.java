 ...
 * @日期: 09-01-30
 */
public class AA extends Applet implements Runnable,MouseListener,MouseMotionListener,KeyListener{
    private final int SIZE_NUM = 30;//二维游戏世界的大小,共SIZE*SIZE个格子
    private final int CELL_Size =10;//每个格式的边长，Java坐标系单位。
    private Color cell =new Color(20,98,40);
    private Color space =new Color(226,245,226);    
    //当代的状况，格子中是否有生命
    private boolean[][] table = new boolean[SIZE_NUM][SIZE_NUM];
    //格子的邻居数目
    private int[][] neighbors = new int[SIZE_NUM][SIZE_NUM];
     ...
     */
    public void getNum() { 
        for (int r = 0; r < SIZE_NUM; r++){//row
            for (int c = 0; c < SIZE_NUM; c++){//col
                if(r-1 >= 0 && c-1 >= 0   && table[r-1][c-1] )neighbors[r][c]++;
                if(r-1 >= 0     && table[r-1][c])             neighbors[r][c]++;
                if(r-1 >= 0 && c+1 < SIZE_NUM && table[r-1][c+1])neighbors[r][c]++;
                if(c-1 >= 0   && table[r][c-1]) neighbors[r][c]++;
                if(c+1 < SIZE_NUM && table[r][c+1]) neighbors[r][c]++;
                if(r+1 < SIZE_NUM&& table[r+1][c]) neighbors[r][c]++;
                if(r+1 < SIZE_NUM && c+1 < SIZE_NUM && table[r+1][c+1])    neighbors[r][c]++;
                if(r+1 < SIZE_NUM && c-1 >=0 && table[r+1][c-1])       neighbors[r][c]++;
            }
        }            
    }
